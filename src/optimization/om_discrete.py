...
def create_model():
    ...
    model.sCashAssets = pe.Set()
    model.sAssets = model.sCashAssets | model.sNonCashAssets
    model.sScenarios = pe.Set(within=pe.Integers)
    
    # Parameters
    model.pPrices = pe.Param(model.sScenarios, model.sNonCashAssets, model.sTime, mutable=True, within=pe.NonNegativeReals) # return as R=(1+r)
    model.pInitialAllocations = pe.Param(model.sAssets, mutable=True, within=pe.NonNegativeIntegers) # shorting not allowed
    ...
    model.pNScenarios = pe.Param(within=pe.NonNegativeIntegers)
    model.pTradeFee = pe.Param(mutable=True, within=pe.NonNegativeReals, default=0)
    
    # Variables
    model.vCashAllocations = pe.Var(model.sScenarios, model.sCashAssets, model.sTime, domain=pe.NonNegativeReals, initialize=0) # shorting not allowed
    ...
    model.vCashTrades = pe.Var(model.sScenarios, model.sCashAssets, model.sNonFinalTime, domain=pe.Reals, initialize=0)
    model.vCashTradesAbs = pe.Var(model.sScenarios, model.sCashAssets, model.sNonFinalTime, domain=pe.NonNegativeReals, initialize=0)
    ...
    def c00_objective_function(model, t):
        return model.Objective == sum(model.vTotalWealth[s,t] for s in model.sScenarios)/model.pNScenarios

    # c01: Initial asset allocations
    def c01_initial_non_cash_allocations(model, a, t):
        return model.vNonCashAllocations[a,t] == model.pInitialAllocations[a]
        
    # c01: Initial asset allocations
    def c02_initial_cash_allocations(model, s, a, t):
        return model.vCashAllocations[s,a,t] == model.pInitialAllocations[a]
    ...
    def c04_allocations_evolution_cash(model, s, a, t):
        return model.vCashAllocations[s,a,t] == model.vCashAllocations[s,a,t-1] + model.vCashTrades[s,a,t-1] + model.pIncome[s,t-1] - model.pExpense[s,t-1]
    
    def c05_absolute_value_cash_trade_positive(model, s, a, t):
        return model.vCashTrades[s,a,t] <= model.vCashTradesAbs[s,a,t]
    
    def c06_absolute_value_cash_trade_negative(model, s, a, t):
        return -model.vCashTrades[s,a,t] <= model.vCashTradesAbs[s,a,t]
    ...
    def c09_self_financing(model, s, t):
        return sum(model.vNonCashTrades[a,t]*model.pPrices[s,a,t] for a in model.sNonCashAssets) + sum(model.vCashTrades[s,a,t] for a in model.sCashAssets) + sum(model.vNonCashTradesAbs[a,t]*model.pPrices[s,a,t]*model.pTradeFee for a in model.sNonCashAssets) + sum(model.vCashTradesAbs[s,a,t]*model.pTradeFee for a in model.sCashAssets) <= 0
    ...
    model.c02_initial_cash_allocations = pe.Constraint(model.sScenarios, model.sCashAssets, model.sInitialTime, rule=c02_initial_cash_allocations)
    ...
    model.c04_allocations_evolution_cash = pe.Constraint(model.sScenarios, model.sCashAssets, model.sNonInitialTime, rule=c04_allocations_evolution_cash)
    model.c05_absolute_value_cash_trade_positive = pe.Constraint(model.sScenarios, model.sCashAssets, model.sNonFinalTime, rule=c05_absolute_value_cash_trade_positive)
    model.c06_absolute_value_cash_trade_negative = pe.Constraint(model.sScenarios, model.sCashAssets, model.sNonFinalTime, rule=c06_absolute_value_cash_trade_negative)
